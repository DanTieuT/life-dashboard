// Shared Plaid sync engine, used by:
//   • plaid-sync.js     — scheduled, hourly, syncs every linked item
//   • plaid-webhook.js  — Plaid pushes SYNC_UPDATES_AVAILABLE, syncs one item
//
// For each item passed in: refresh account balances and pull transaction
// changes via /transactions/sync (cursor-based, so each call only sees what
// changed since last time). Mapped transactions go into appData.transactions
// (deduped by plaidTxnId, transfers excluded, backfill capped at 30 days).
//
...
// res.removed; reconcilePending() swaps the posted data onto the existing row
// (matched via pendingPlaidTxnId) so it's updated in place, not duplicated.
//
...
// get a second look against any contribution-tracked goal's `autoMatchKeyword`
// (case-insensitive substring on the raw Plaid `name`, e.g. "schwab"). A match
// appends {date, amount, plaidTxnId} to that goal's `contributions` array
// instead — never to appData.transactions, so it never counts as spending.
// The `!pt.pending` guard makes it wait for the posted version so a goal is
// never funded twice.
...
// itemDocs: Firestore QueryDocumentSnapshots from users/{uid}/plaidItems.
// Returns { summary, balancesUpdated, txnsAdded, txnsRemoved, contribsAdded }.
async function syncItems(db, itemDocs) {
...
module.exports = { initFirebase, syncItems, USER_UID, BACKFILL_DAYS };
